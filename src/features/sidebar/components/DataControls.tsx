...
export const DataControls = () => {
    ...
    const handleKNeighbourChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const k = parseInt(e.target.value, 10)
        if(!isNaN(k) && k >= 1) setKNeighbours(k)
    }
    ...
    return (
        ...
        {/* K-Neighbours selector */}
        <div>
            <label htmlFor="kNeighbours-input" className="block text-sm font-medium mb-1">
                K-Neighbours (link count)
            </label>
            <p className="text-xs text-gray-400 mb-2">Number of nearest neighbours to link to</p>
            <input 
                id='k-neighbours-input'
                type="number"
                min={1}
                max={rawNodes.length - 1}
                value={kNeighbours}
                onChange={handleKNeighbourChange}
                className='block p-2 bg-gray-700 border-gray-800 rounded text-sm'
            />
        </div>
    </div>
);
